...
Products can now be linked to a **Consignor** (Supplier) via the `consignor_id` field.

### Usage

- **Filtering**: Use `GET /api/v1/products?consignor_id={UUID}` to list products for a specific supplier.
- **Creation/Update**: Pass `consignor_id` in the request body. Send `null` to unlink.
